public class TicTacToe extends GameSearch{
	...
	*/
	...
	private boolean winCheck(int i1, int i2, int i3,
		...
		// are you a human or a program player?
        int b = 0;
        if (player){
        	b = TicTacToePosition.HUMAN;
        }
        else {
        	b = TicTacToePosition.PROGRAM;
        }       
        if (pos.board[i1] == b &&
            pos.board[i2] == b &&
    ...
    /*
	* The position evaluation is
	* going to return a position evaluation
	* for a board postion and a player.
	* Note that if we call positionEvaluation
	* switching the player for the same board position, then the value
	* returned is the negative of the value calculated for the opposing player
    */
    ...
    /*
	* method call with a current position and a side to move(i.e. human or program)
    */
    public Position [] possibleMoves(Position p, boolean player) {
        if (GameSearch.DEBUG) System.out.println("posibleMoves("+p+","+player+")");
        TicTacToePosition pos = (TicTacToePosition)p;
        int count = 0;
        //first count the blank spaces	
        for (int i=0; i<9; i++){
		  	if (pos.board[i] == 0) count++;	
        } 
        if (count == 0) {
        ...
        for (int i=0; i<9; i++) {
            if (pos.board[i] == 0) {
                ...
                //if you are the HUMAN you can move to a 1 if you are PROGRAM you
                //move to -1. 
                if (player) {
                	pos2.board[i] = 1; 
                } else {
                ...
                //copy the position found to ret
                ret[count++] = pos2; 
